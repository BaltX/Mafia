...
public class DoctorVotingHandler : IVotingHandler
{
    public GameStage RequiredStage => GameStage.DoctorTurn;

    public bool CanVote(LobbyState lobby, PlayerState player) =>
        lobby.Stage == GameStage.DoctorTurn && !player.IsDead() && player.IsDoctor();
    ...
    private static bool ValidateTarget(LobbyState lobby, PlayerState player, Guid targetId)
    {
        ...
        return target is not null && target.IsAlive && !target.IsHost();
    }
}